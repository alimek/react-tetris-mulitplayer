...
export const killRows = (rows: any[]): {
  wallsLine: number;
  rows: number[][],
  numRowsKilled: number;
} => {
  const newRows: any[] = [];
  const rowsToClear: any[] = [];
  let wallsLine: number = 0;
  ...
  const wallsToBeRemoved = getWallsNumberToBeRemoved(
    wallsLine,
    rowsToClear.length,
  );
...
export const addLine = (rows: number[][], howMany: number = 1): number[][] => {
